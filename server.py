...
host = ''
port = 2839
accounts = [['ndang', 'natepassword', 0], ['nnguyen', 'nickpassword', 0], ['ntran', 'nampassword', 0]]
addresses = []
buffers = []
messages = []
...
while 1:
	...
	if data[0] == '0':
		for i in range(len(accounts)):
			if accounts[i][0] == data[1:]:
				if accounts[i][2] == 0:
					addresses.append([client_addr, i])
					reply = '0'
				else:
					j = len(addresses) - 1
					while j >= 0:
						if addresses[j][0] == client_addr:
							addresses.pop(j) #
						j = j - 1	
					reply = '1'
				break
		...
		for i in range(len(accounts)):
			if accounts[i][0] == data[1:]:
				buffers.append([client_addr, data[1:]])
				reply = '0'
				break
		if i >= len(accounts):
			reply = '1'
	...
	s.sendto(reply, client_addr) #send reply back to clients
